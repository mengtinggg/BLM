...
def crawl_profile(session, base_url, profile_url, post_limit):
    ...
    scraped_posts = list()
    posts_id = None
    with open('BlackLivesMatter_Facebook2.json', 'w') as json_file:
        while n_scraped_posts < post_limit:
            ...
            for post_url in posts_urls:
                ...
                n_scraped_posts += 1
                if n_scraped_posts % 50 == 0:
                    print("Posts scraped", n_scraped_posts)
...
def extract_comments(session, base_url, post_bs, post_url):
    ...
    logging.info('Scraping from new post')
    while True:
...
def save_data(data):
    ...
    """
    with open('profile_posts_data.json', 'a') as json_file:
...
if __name__ == "__main__":
    ...
    credentials = {'email':'enter facebook id', 'pass':'enter facebook password'}
    profiles_urls = ["https://mobile.facebook.com/BlackLivesMatter/posts/?ref=page_internal&_rdr"]
    ...
    for profile_url in profiles_urls:
        posts_data = crawl_profile(session, base_url, profile_url, 1000)
        profile_count += 1
        print("Profile Scraped =", profile_count)
